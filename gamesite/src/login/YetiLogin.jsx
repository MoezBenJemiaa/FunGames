...
const YetiLogin = () => {
  ...
  useEffect(() => {
    ...
    const showPasswordCheck = showPasswordCheckRef.current;
    ...
    const elements = {
      armL: select(".armL"),
      armR: select(".armR"),
      twoFingers: select(".twoFingers"),
      eyeL: select(".eyeL"),
      eyeR: select(".eyeR"),
      nose: select(".nose"),
      mouth: select(".mouth"),
      mouthBG: select(".mouthBG"),
      mouthSmallBG: select(".mouthSmallBG"),
      mouthMediumBG: select(".mouthMediumBG"),
      mouthLargeBG: select(".mouthLargeBG"),
      mouthOutline: select(".mouthOutline"),
      tooth: select(".tooth"),
      tongue: select(".tongue"),
      chin: select(".chin"),
      face: select(".face"),
      eyebrow: select(".eyebrow"),
      outerEarL: select(".earL .outerEar"),
      outerEarR: select(".earR .outerEar"),
      earHairL: select(".earL .earHair"),
      earHairR: select(".earR .earHair"),
      hair: select(".hair"),
    ...
    TweenMax.set(armR, { x: 93, y: 220, rotation: -105 });
    ...
    const eyeLCoords = getCenterCoords(eyeL);
    const eyeRCoords = getCenterCoords(eyeR);
    const noseCoords = getCenterCoords(nose);
    const mouthCoords = getCenterCoords(mouth);
    const emailCoords = getCenterCoords(email);
    ...
    let mouthAngle, mouthX, mouthY, mouthR;
    let chinX, chinY, chinS;
    let faceX, faceY, faceSkew;
    ...
    const handlePasswordFocus = () => coverEyes();
    const handlePasswordBlur = () => uncoverEyes();
    ...
    showPasswordCheck.addEventListener("change", togglePassword);

    TweenMax.set(armL, { x: -93, y: 220, rotation: 105 });
    TweenMax.set(armR, { x: 93, y: 220, rotation: -105 });
    TweenMax.set(mouth, { transformOrigin: "center center" });
    TweenMax.set(bodyBGchanged, { display: "none" });
    TweenMax.set(mouthMediumBG, { display: "none" });
    TweenMax.set(mouthLargeBG, { display: "none" });
    startBlinking(5);
